...
int acc_calibration_value = -9791;                            //Enter the accelerometer calibration value

//Various settings
float pid_p_gain = 19;                                       //Gain setting for the P-controller (15)
float pid_i_gain = 0.3;                                       //Gain setting for the I-controller (1.5)
float pid_d_gain = 8;                                       //Gain setting for the D-controller (30)
float turning_speed = 40;                                    //Turning speed (20)
float max_target_speed = 110;                                //Max target speed (100)
...
void setup(){
  ...
  for(receive_counter = 0; receive_counter < 530; receive_counter++){       //Create 500 loops
  ...
  gyro_pitch_calibration_value /= 530;                                      //Divide the total value by 500 to get the avarage gyro offset
  gyro_yaw_calibration_value /= 530;                                        //Divide the total value by 500 to get the avarage gyro offset
...
void loop() {
  ...
  if(received_counter <=62) received_counter++;
  else received_byte = 0;
 ...
 ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(received_byte == 76){                                                  //If the first bit of the receive byte is set change the left and right variable to turn the robot to the left
    pid_output_left += turning_speed;                                       //Increase the left motor speed
    pid_output_right -= turning_speed;                                      //Decrease the right motor speed
  }
  if(received_byte == 82){                                                  //If the second bit of the receive byte is set change the left and right variable to turn the robot to the right
    pid_output_left -= turning_speed;                                       //Decrease the left motor speed
    pid_output_right += turning_speed;                                      //Increase the right motor speed
  }

  if(received_byte == 70){                                            //If the third bit of the receive byte is set change the left and right variable to turn the robot to the right
  ...
  if(received_byte == 66){                                            //If the forth bit of the receive byte is set change the left and right variable to turn the robot to the right
  ...
  if(received_byte == 0){                                                    //Slowly reduce the setpoint to zero if no foreward or backward command is given
